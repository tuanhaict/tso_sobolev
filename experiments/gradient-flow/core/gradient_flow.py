...
def SbTS(X, Y, theta, intercept, mass_division = 'distance_based', p = 2, delta = 2., device = 'cuda'):
    ...
    return TWD_obj(X, Y, theta, intercept)
def OSbTS(X, Y, theta, intercept, mass_division = 'distance_based', p = 2, delta = 2., device = 'cuda', n_function="exp", p_agg=2, optimization_method="bounded"):
    # print(p)
    # print(delta)
    # exit()
    L = theta.shape[0]
    nlines = theta.shape[1]
    TWD_obj = OSb_TSConcurrentLines(p=p, delta=delta, mass_division=mass_division, device=device, n_function=n_function, p_agg=p_agg, optimization_method=optimization_method)
    return TWD_obj(X, Y, theta, intercept)
