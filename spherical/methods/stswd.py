...
def stswd(X, Y, ntrees=250, nlines=4, p=2, delta=2, device='cuda', type='normal'):
    TW_obj = STSWD(ntrees=ntrees, nlines=nlines, p=p, delta=delta, device=device, type=type)
    stswd = TW_obj(X, Y)
    return stswd

def stswd_unif(X, ntrees=250, nlines=4, p=2, delta=2, device='cuda', type='normal'):
    Y_unif = unif_hypersphere(X.shape, device=X.device) 
    stswd_unif = stswd(X, Y_unif, ntrees, nlines, p, delta, device, type)
    return stswd_unif